...
class GuildMemoryStore:
    ...
    def read_for_prompt(self, guild_id: int) -> str:
        ...
        try:
            ...
            mtime = 0.0
        cached = self._prompt_cache.get(guild_id)
        if cached is not None and cached[0] == mtime:
            LOGGER.debug("guild memory cache hit guild_id=%s", guild_id)
            return cached[1]

        content = self.read_for_guild(guild_id)
        if not content:
            rendered = "Server memory:\n(none)\n"
        else:
        ...
        self._prompt_cache[guild_id] = (mtime, rendered)
        LOGGER.debug("guild memory cache miss guild_id=%s", guild_id)
        return rendered
